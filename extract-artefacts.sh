...
### Browser & cache
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.cache 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.mozilla 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.java/deployment/cache/ 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.dropbox/*.db* 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.npm/ 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.recently-used* 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.wget-hsts 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.local/share/zeitgeist/activity.sqlite 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/cache_home.tar "$homeuser"/.local/share/zeitgeist/activity.sqlite-wal 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
gzip /tmp/artefacts/cache_home.tar

while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.bashrc 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.bash_profile 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.bash_logout 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.profile 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.cshrc 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.ksh 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.tcsh 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.zlogin 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.zlogout 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.zprofile 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.logout 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.login 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
while IFS= read -r homeuser;do tar vuf /tmp/artefacts/bash_home.tar "$homeuser"/.k5login 2> /dev/null;done < <(awk -F ':' '{print $(NF-1)}' /etc/passwd)
...
if [ $OS == 1 ] && [ $DUMP_LOG == 1 ]; then find / -path /run/log -prune -o -path /etc -prune -o -path /sys -prune -o -path /var/log -prune -o \( -fstype nfs -prune \) -o \( -fstype sysfs -prune \) -o \( -fstype proc -prune \) -o -name '*.log' -o -name '*.log.*' -o -name 'catalina.out' |grep -v '^/var/log'|grep -v '^/run/log'|tar -zcpvf /tmp/artefacts/otherlog.tar.gz --files-from -;fi
...
then
  ...
  if openssl cms -encrypt -stream -binary -aes-256-cbc -in /tmp/artefacts-"$(hostname)".tgz -out /tmp/artefacts-"$(hostname)".tgz.enc -recip /tmp/toolsEAL/tools/pub_key -keyopt rsa_padding_mode:pkcs1 -keyopt rsa_padding_mode:oaep -outform DER /tmp/toolsEAL/tools/pub_key
  then
